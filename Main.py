...
def Simulate(ClusterTest=False,Progress=True):
    #Allow for parameter inputs
    ...
    ConsumerSurplus = config.ConsumerSurplus
    initialM = config.startingM
    ...
    with tqdm(total=Stationarity_Target, desc="Tracking Stationarity",disable=not Progress) as pbar:
        while min(Firm_Stationarity) < Stationarity_Target:
            ...
            #Consumer Surplus
            CS_T,CS_R,M_Expected = ConsumerSurplus(Price_Actions,Round,Industry_m,Leader)
            ...
            if Round % 20000 == 0 and Progress:
                ...
                pbar.refresh()
            Round+=1
    if len(Price_statlog)!=Stationarity_Target:print("Stat log not equal to target")

    #Process results
    if args.plot or not ClusterTest:
        ...
        for i, f in enumerate(Firms):
            ...
            for state in np.ndindex(f.state_shape):
                ...
                # Count how many actions share this maximum value
